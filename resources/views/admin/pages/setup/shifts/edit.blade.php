...
<div class="max-w-3xl mx-auto p-6">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold">Edit Shift: {{ $shift->name }}</h1>
        <p class="text-sm text-gray-500">Update the details for this shift.</p>
    </div>

    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
        <form method="POST" action="{{ route('admin.pages.setup.shifts.update', $shift->id) }}" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input
                    ...
                    name="name"
                    type="text"
                    class="mt-1 block py-1 w-full rounded-lg border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 @error('name') border-red-500 @enderror"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        ...
                        name="start_time"
                        type="time"
                        class="mt-1 block py-1 w-full rounded-lg border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 @error('start_time') border-red-500 @enderror"
                        {{-- Laravel's time field stores HH:MM:SS, so we format it for the time input (HH:MM) --}}
                ...
                <div>
                    ...
                    <input
                        ...
                        name="end_time"
                        type="time"
                        class="mt-1 block py-1 w-full rounded-lg border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 @error('end_time') border-red-500 @enderror"
            ...
            <div>
                ...
                <select
                    id="actor"
                    name="actor"
                    class="mt-1 block w-full rounded-lg border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 @error('actor') border-red-500 @enderror"
                    required
                >
            ...
            <div>
                ...
                <input
                    ...
                    min="0"
                    step="5"
                    class="mt-1 block py-1 w-40 rounded-lg border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 @error('break_minutes') border-red-500 @enderror"
                    placeholder="e.g., 30"
                    value="{{ old('break_minutes', $shift->break_minutes ?? 30) }}"
                />
                @error('break_minutes') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                <p class="mt-1 text-xs text-gray-500">Use total minutes.</p>
            </div>
            ...
            <div class="pt-2">
                <label class="inline-flex items-center gap-3">
                    <input
                        ...
                        @checked(old('crosses_midnight', (bool) ($shift->crosses_midnight)))
                    />
            ...
            <div class="flex items-center justify-between pt-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    {{-- Delete Form (Included here for convenience in the edit view) --}}
                    <form method="POST" action="{{ route('admin.pages.setup.shifts.delete', $shift->id) }}" onsubmit="return confirm('WARNING: Are you sure you want to permanently delete the shift: {{ $shift->name }}?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
